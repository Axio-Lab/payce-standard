...
fn wrapped_sol_pubkey() -> Pubkey {
    Pubkey::from_str(WRAPPED_SOL).expect("WSOL mint constant")
}
...
async fn swap_input_balance_human(
    ...
    match route_id {
        7 | 8 | 9 => get_native_sol_balance(rpc, wallet_address).await,
...
fn swap_amount_covered(route_id: usize, balance: f64, amount: f64) -> bool {
    let tol = if matches!(route_id, 7 | 8 | 9) {
        1e-9
    } else {
        1e-6
    };
...
fn swap_route_menu_body(page: usize) -> String {
    ...
    let total_pages = std::cmp::max(1, (SWAP_ROUTE_COUNT + page_size - 1) / page_size);
...
pub async fn handle_defi(
    ...
    match inputs[1].as_str() {
        "1" => {
            handle_swap_token(pool, redis, rpc, config, user_id, user_phone, &inputs[2..]).await
        }
        _ => {
        ...
        DataPlanNav::Picked {
            ...
            if tail.len() == 1 {
                ...
                if human_to_raw(amount, dec).is_none() {
                    return format!("END Amount is too small or too large for this route.");
                }
            ...
            if tail.len() == 2 {
                ...
                match run_swap(&http, config, &kp, &in_s, &out_s, raw).await {
                    Ok(sig) => {
                    ...
                    Err(e) => {
                        log::error!("[DeFi] Jupiter swap failed: {e}");
